...
function parseNested(x: any, out: Record<string, any>, nestedRule: ValidationRule, keys: string[], propertyPath: string, isQuery: boolean | undefined): void {
  for (const key of keys) {
    const value = validate((x as Record<string, any>)[key], nestedRule, `${propertyPath}.${key}`, isQuery);
...
export const objectHandler = (x: any, rule: Partial<ObjectRule>, propertyPath: string, isQuery?: boolean): object => {
  ...
  if (rule.schema) {
    ...
    for (const [key, schemaRule] of entries) {
      const value = validate((x as Record<string, any>)[key], schemaRule, `${propertyPath}.${key}`, isQuery);
